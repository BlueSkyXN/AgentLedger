...
这些策略用于让同一事件在重复导入或 v2 数据库合并时保持稳定主键。完整源 JSON 只通过瞬时 `FingerprintJSON` 参与首次 fingerprint，不落库。对于一行只产生一个 usage event 的 Codex 日志，`import` 还会用 `source_file + line_number + raw_sha256` 识别同一原始 JSONL 行，并把当前 `event_id` 精确匹配行和同来源 sibling 联合收敛；默认脱敏导出中的候选可额外用 session、timestamp、line 和 raw hash 受限匹配。reconciliation 只比较结构化统计、source identity 和 model classification，不依赖 `raw_usage_json`。已有或 incoming `raw_hash` / `fallback` 事件保留已计算 ID/strategy，避免清除 raw 后 identity 漂移。其他 adapter 可能从一行拆出多个合法事件，不使用这一兼容身份。
...
6. 在同一 destination transaction 内插入本地未见过的 `usage_events`，所有 incoming `raw_usage_json` 均省略并写为 `NULL`。
...
当前工具不主动联网。隐私风险主要来自本地数据库和导出的 `.aldb` 文件。statistics-only 新写入要求 `usage_events.raw_usage_json` 为 `NULL`；旧库在运行显式迁移前仍可能含 legacy 或 compact raw。在分享数据库、截图或面板结果前，仍需把它当作私有使用数据处理。

当前 `export` 默认会在导出副本中清空路径字段和历史 `raw_usage_json`。设计中的 cleanup/quarantine、加密 raw archive 尚未落地到当前 CLI。

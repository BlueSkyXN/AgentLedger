...
// UsageEvent is the v3 fact-table representation of one source-backed usage event.
// It deliberately contains no conversation body, recorded money, device data, or
// request-performance timing.
type UsageEvent struct {
	EventID          string `json:"event_id" db:"event_id"`
	IdentityVersion  int    `json:"identity_version" db:"identity_version"`
	IdentityStrategy string `json:"identity_strategy" db:"identity_strategy"`
	IdentityScope    string `json:"identity_scope" db:"identity_scope"`
	ContentSHA256    string `json:"content_sha256" db:"content_sha256"`
	ParserVersion    string `json:"parser_version" db:"parser_version"`
	EventGranularity string `json:"event_granularity" db:"event_granularity"`

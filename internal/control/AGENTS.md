...
`internal/control/` serves the local read-only HTTP API and static Web panel assets. Read this card before modifying `/api/v2/*`, filter parsing, pagination, error responses, static file handling, config/status/health snapshots, or path redaction. Key files: `server.go`, `server_test.go`, plus `cmd/serve.go` for listener behavior.

## Local invariants

- API methods are GET-only. Non-GET requests must not mutate local state.
- API responses must not expose raw source usage; config and DB paths should be home-redacted where intended.
